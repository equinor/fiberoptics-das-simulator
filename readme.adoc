...
APPLY ignores the request body except for Custom.das-simulator-profile.
The simulator loads the matching profile from REMOTE_CONTROL_PROFILES_DIR and uses that configuration
to start producing data. A new AcquisitionId and MeasurementStartTime are generated on every APPLY,
so stored profile values are not reused. STOP sends a best-effort stop signal to streaminitiator
for the last applied acquisition id. Example profiles are included in the remote-control-profiles folder.
